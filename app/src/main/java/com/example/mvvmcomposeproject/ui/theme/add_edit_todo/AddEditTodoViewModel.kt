package com.example.mvvmcomposeproject.ui.theme.add_edit_todo
...
class AddEditTodoViewModel @Inject constructor(private val repository:TodoRepository,savedStateHandle:SavedStateHandle):ViewModel() {
    ...
    init {
        ...
        if (todoId != -1){//if is there todo it equals to -1
        ...
        when(event){
            ...
            is AddEditTodoEvent.OnSaveTodoClick->{
                viewModelScope.launch {
                    ...
                    repository.insertTodo(Todo(title=title,description=description,isDone=todo?.isDone ?: false ,id = todo?.id))
